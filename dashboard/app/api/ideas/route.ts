...
const LOCK_PATH = path.join(os.tmpdir(), "arxiv-ideas.lock");
...
export async function POST() {
  ...
  try {
    ...
    await execFileAsync(PIPELINE_BIN, ["--ideas", "--vault", VAULT_ROOT], {
      env: { ...process.env, ANTHROPIC_API_KEY: apiKey },
      timeout: 3 * 60 * 1000,
    });

    // Local date, matching the pipeline's Ideas/YYYY-MM-DD.md filename.
    const now = new Date();
    const today = [
      now.getFullYear(),
    ...
    const notePath = path.join(VAULT_ROOT, "Ideas", `${today}.md`);
    return NextResponse.json({ status: "ok", path: notePath });
  } catch (err) {
